...
import com.lzq.echarts.code.Easing;
import com.lzq.echarts.code.Symbol;
import com.lzq.echarts.code.Trigger;
import com.lzq.echarts.option.NoDataLoadingOption;
import com.lzq.echarts.series.Parallel;
import com.lzq.echarts.series.Series;
import com.lzq.echarts.style.ItemStyle;
...
public class Option {
	...
	/**
	 * 非IE8-支持渲染为图片，（详见renderAsImage） {boolean | string}
	 * false，非IE8-支持渲染为图片，可设为true或指定图片格式（png |
	 * jpeg），渲染为图片后实例依然可用（如setOption，resize等），但各种交互失效
	 */
	private Object renderAsImage;
	/**
	 * 是否启用拖拽重计算特性，默认关闭，（详见calculable，相关的还有 calculableColor， calculableHolderColor，
	 * nameConnector， valueConnector）
	 */
	private Boolean calculable;
	/**
	 * 是否启用图表初始化动画，默认开启，建议IE8-关闭，（详见 animation，相关的还有 addDataAnimation，
	 ...
	 * @return
	 */
	public Option legend(List values) {
