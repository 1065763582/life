package com.lzq.mgenerator.plugins;
...
public class CommentPlugin extends PluginAdapter {
	...
	private void comment(JavaElement element, IntrospectedTable introspectedTable,
	...
	void commentResultMap(XmlElement element, IntrospectedTable introspectedTable) {
		...
		if (!es.isEmpty()) {
			...
			HashMap<XmlElement, TextElement> map = new HashMap<>();
			...
			while (true) {
					...
					if (e instanceof TextElement) {
						...
						if (!as.isEmpty()) {
							...
							if (col != null) {
								...
								Optional<IntrospectedColumn> icop = introspectedTable.getColumn(col);
								IntrospectedColumn ic = icop.get();
								if (ic != null) {
				...
				Set<XmlElement> set = map.keySet();
				Iterator<XmlElement> i$ = set.iterator();

				while (i$.hasNext()) {
					XmlElement e = (XmlElement) i$.next();
					int id = es.indexOf(e);
					es.add(id, (TextElement) map.get(e));
